...
describe("Area Controller Integration Tests", () => {
  ...
  describe("GET /areas", () => {
    it("should return all areas", async () => {
      ...
      const response = await request(app).get("/areas").expect(200);
  ...
  describe("GET /areas/:id", () => {
    it("should return a specific area by ID", async () => {
      ...
      const response = await request(app).get(`/areas/${area.id}`).expect(200);
    ...
    it("should return 404 when area ID does not exist", async () => {
      await request(app).get("/areas/nonexistent-id").expect(404);
    });
  });
  ...
  describe("POST /areas", () => {
    it("should create a new area with valid data", async () => {
      ...
      const savedArea = await areaRepository.findOne({
    ...
    it("should reject area creation with missing required fields", async () => {
      ...
      await request(app).post("/areas").send(invalidArea).expect(400);
    });
  ...
  describe("PUT /areas/:id", () => {
    it("should update an existing area", async () => {
      ...
      const updatedArea = await areaRepository.findOne({
        where: { id: area.id },
      });
  ...
  describe("DELETE /areas/:id", () => {
    it("should delete an existing area", async () => {
      ...
      await request(app).delete(`/areas/${area.id}`).expect(204);

      // Verify area was deleted
      const deletedArea = await areaRepository.findOne({
        where: { id: area.id },
      });
    ...
    it("should return 404 when deleting non-existent area", async () => {
      await request(app).delete("/areas/nonexistent-id").expect(404);
    });
  });
});
